...
```bash
# Windows (PowerShell)
OVIS_URL="http://192.168.178.151:5678/webhook/mcp"
OVIS_TIMEOUT="8.0"
```
...
- **參數**
  ...
  - 簍空：立方體 / 球體 / 不規則圖形形可設定「有孔」與孔寬 / 孔高（方塊不支援簍空）
- **操作**
  - 左鍵點擊：選取；點空白處取消。
  - Shift + 左鍵拖曳：旋轉選取物。
  - 右鍵拖曳：XZ 平面移動。
...
3. (執行`auto_control.py`)點擊**辨識參數**：從攝影機拍照、回送 OVIS 辨識描述，自動套表單並生成。
